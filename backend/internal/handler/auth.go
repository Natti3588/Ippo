...
func (h *AuthHandler) AuthSignup(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.logger.InfoContext(r.Context(), "リクエストを解釈できません", "error", err, "op", "signup")
	...
	if err != nil {
		if e, ok := errors.AsType[*domain.InvalidInputError](err); ok {
			h.logger.InfoContext(r.Context(), "入力が不正", "error", err, "op", "signup")
			writeProblem(r.Context(), w, h.logger, http.StatusBadRequest, e.Detail)
			return
		}
		if errors.Is(err, domain.ErrEmailTaken) {
			writeProblem(r.Context(), w, h.logger, http.StatusConflict, "このメールアドレスは既に使われています")
			return
		}
		h.logger.ErrorContext(r.Context(), "サインアップに失敗", "error", err)
...
func (h *AuthHandler) AuthLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.logger.InfoContext(r.Context(), "リクエストを解釈できません", "error", err, "op", "login")
		writeProblem(r.Context(), w, h.logger, http.StatusBadRequest, "リクエストの形式が不正です")
		return
	}
	...
	if err != nil {
		if errors.Is(err, domain.ErrInvalidCredentials) {
			writeProblem(r.Context(), w, h.logger, http.StatusUnauthorized, "メールアドレスまたはパスワードが違います")
			return
		}
		h.logger.ErrorContext(r.Context(), "ログインに失敗", "error", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func (h *AuthHandler) AuthLogout(w http.ResponseWriter, r *http.Request) {
	if c, err := r.Cookie(sessionCookieName); err == nil && c.Value != "" {
		if err := h.svc.Logout(r.Context(), c.Value); err != nil {
			h.logger.ErrorContext(r.Context(), "ログアウトに失敗", "error", err)
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
	}
...
func (h *AuthHandler) MeUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.logger.InfoContext(r.Context(), "リクエストを解釈できません", "error", err, "op", "meUpdate")
		writeProblem(r.Context(), w, h.logger, http.StatusBadRequest, "リクエストの形式が不正です")
		return
	}
	...
	if err != nil {
		if e, ok := errors.AsType[*domain.InvalidInputError](err); ok {
			h.logger.InfoContext(r.Context(), "入力が不正", "error", err, "op", "meUpdate")
			writeProblem(r.Context(), w, h.logger, http.StatusBadRequest, e.Detail)
			return
		}
		h.logger.ErrorContext(r.Context(), "表示名の変更に失敗", "error", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
